...
COSIM_SCRIPT_DIR ?= $(ROOT_DIR)/Interface_scripts/cosim
...
.PHONY: help bootstrap cosim dirs 3dice-build softhier-init softhier-power-check ice-inputs softhier-build coupled-run coupled-status coupled-stop clean-run clean-latest clean-runs list-runs latest-run adapter-smoke

help:
	@printf '%s\n' \
		'SoftHier/3D-ICE root orchestration targets:' \
		'  make bootstrap       First-time setup: submodules, SoftHier hooks, 3D-ICE build' \
		'  make cosim           Run bootstrap, then a timestamped coupled simulation' \
...
bootstrap:
	...
	PYTHON="$(PYTHON)" \
	"$(COSIM_SCRIPT_DIR)/bootstrap.sh"

cosim: bootstrap coupled-run
...
3dice-build:
	@if [[ "$(BUILD_3DICE)" == "1" && ( ! -x "$(DICE_BIN_DIR)/3D-ICE-Server" || ! -x "$(DICE_BIN_DIR)/3D-ICE-Client" ) ]]; then \
		SRC_DIR="$(DICE_DIR)" "$(COSIM_SCRIPT_DIR)/3dice_client_server.sh" install; \
	fi
...
softhier-power-check:
	@"$(COSIM_SCRIPT_DIR)/bootstrap.sh" --check-only

ice-inputs: dirs
	$(MAKE) -C "$(SOFTHIER_DIR)" ice_prepare \
		cfg="$(CFG)" \
...
coupled-run:
	...
	DONE_FILE="$(DONE_FILE)" \
	"$(COSIM_SCRIPT_DIR)/coupled_run.sh" run

coupled-status:
	@RUN_DIR="$(RUN_DIR)" PID_DIR="$(PID_DIR)" "$(COSIM_SCRIPT_DIR)/coupled_run.sh" status

coupled-stop:
	@RUN_DIR="$(RUN_DIR)" PID_DIR="$(PID_DIR)" "$(COSIM_SCRIPT_DIR)/coupled_run.sh" stop
...
adapter-smoke:
	$(PYTHON) -m py_compile \
		"$(COSIM_SCRIPT_DIR)/ice_trace_adapter.py" \
		"$(COSIM_SCRIPT_DIR)/wait_for_log.py"
